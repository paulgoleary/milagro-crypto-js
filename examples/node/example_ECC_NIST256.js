...
var PW = ctx.Utils.stringtobytes(pp);
// private key S0 of size EGS bytes derived from Password and Salt 
var S0 = ctx.ECDH.PBKDF2(sha, PW, SALT, 1000, EGS);

console.log("Alice's private key= 0x" + ctx.Utils.bytestohex(S0));
// Generate Key pair S/W 
ctx.ECDH.KEY_PAIR_GENERATE(null, S0, W0);

console.log("Alice's public key= 0x" + ctx.Utils.bytestohex(W0));
...
console.log("Servers private key= 0x" + ctx.Utils.bytestohex(S1));
console.log("Servers public key= 0x" + ctx.Utils.bytestohex(W1));
...
console.log("Alice's ECDH Key= 0x" + ctx.Utils.bytestohex(KEY));
console.log("Servers ECDH Key= 0x" + ctx.Utils.bytestohex(KEY));
...
if (ctx.ECP.CURVETYPE != ctx.ECP.MONTGOMERY) {
    ...
    console.log("V= 0x" + ctx.Utils.bytestohex(V));
    console.log("C= 0x" + ctx.Utils.bytestohex(C));
    console.log("T= 0x" + ctx.Utils.bytestohex(T));
    ...
    console.log("Message is 0x" + ctx.Utils.bytestohex(M));
    ...
    console.log("C= 0x" + ctx.Utils.bytestohex(CS));
    console.log("D= 0x" + ctx.Utils.bytestohex(DS));
