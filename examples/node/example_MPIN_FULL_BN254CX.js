...
if (PERMITS) {
    ...
    console.log("Time Permit TP: 0x" + ctx.Utils.bytestohex(PERMIT));

    /* This encoding makes Time permit look ctx.RANDom - Elligator squared */
    ctx.MPIN.ENCODING(rng, PERMIT);
    console.log("Encoded Time Permit TP: 0x" + ctx.Utils.bytestohex(PERMIT));
    ctx.MPIN.DECODING(PERMIT);
    console.log("Decoded Time Permit TP: 0x" + ctx.Utils.bytestohex(PERMIT));
} else {
    date = 0;
}
