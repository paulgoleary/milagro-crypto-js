...
BIG_XXX.prototype = {
    ...
    invmod2m: function() {
        ...
        for (var i = 8; i < BIG_XXX.BIGBITS; i <<= 1) {
            ...
            b.copy(this);
            b.mod2m(i);
            var t1 = BIG_XXX.smul(U, b);
            t1.shr(i);
            c.copy(this);
            c.shr(i);
            c.mod2m(i);

            var t2 = BIG_XXX.smul(U, c);
            t2.mod2m(i);
            t1.add(t2);
            t1.norm();
            b = BIG_XXX.smul(t1, U);
...
BIG_XXX.mul = function(a, b) {
    ...
    for (var k = BIG_XXX.NLEN; k < 2 * BIG_XXX.NLEN - 1; k++) {
        s -= d[k - BIG_XXX.NLEN];
        t = s;
        for (i = BIG_XXX.NLEN - 1; i >= 1 + Math.floor(k / 2); i--) t += (a.w[i] - a.w[k - i]) * (b.w[k - i] - b.w[i]);
        c.w[k] = t;
    }
...
BIG_XXX.sqr = function(a) {
    ...
    for (var j = BIG_XXX.NLEN - 1 + BIG_XXX.NLEN % 2; j < BIG_XXX.DNLEN - 3;) {
        t = a.w[BIG_XXX.NLEN - 1] * a.w[j - BIG_XXX.NLEN + 1];
        for (var i = j - BIG_XXX.NLEN + 2; i < (j + 1) >> 1; i++) {
            t += a.w[j - i] * a.w[i];
        }
        t += t;
        c.w[j] = t;
...
BIG_XXX.monty = function(m, nd, d) {
    ...
    t = d.w[0];
    v[0] = ((t & BIG_XXX.BMASK) * nd) & BIG_XXX.BMASK;
    t += v[0] * m.w[0];
    c = d.w[1] + (t * BIG_XXX.MODINV);
    s = 0;

    for (var k = 1; k < BIG_XXX.NLEN; k++) {
        t = c + s + v[0] * m.w[k];
        for (i = k - 1; i > Math.floor(k / 2); i--) t += (v[k - i] - v[i]) * (m.w[i] - m.w[k - i]);
        v[k] = ((t & BIG_XXX.BMASK) * nd) & BIG_XXX.BMASK;
        t += v[k] * m.w[0];
