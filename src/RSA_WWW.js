...
RSA_WWW = {
    ...
    KEY_PAIR: function(rng, e, PRIV, PUB) { /* IEEE1363 A16.11/A16.12 more or less */
        ...
        for (;;) {
            ...
            while (!FF_WWW.prime(PRIV.p, rng)) PRIV.p.inc(4);
        ...
        for (;;) {
            ...
            while (!FF_WWW.prime(PRIV.q, rng)) PRIV.q.inc(4);
        ...
        PUB.n = FF_WWW.mul(PRIV.p, PRIV.q);
        PUB.e = e;
    ...
    OAEP_DECODE: function(sha, p, f) {
        ...
        if (f.length < RSA_WWW.RFS) {
            var d = RSA_WWW.RFS - f.length;
            for (i = RFS - 1; i >= d; i--)
                f[i] = f[i - d];
            for (i = d - 1; i >= 0; i--)
                f[i] = 0;
    ...
    /* RSA_WWW encryption with the public key */
    ENCRYPT: function(PUB, F, G) {
        var n = PUB.n.getlen();
        var f = new FF_WWW(n);

        FF_WWW.fromBytes(f, F);
    ...
    /* RSA_WWW decryption with the private key */
    DECRYPT: function(PRIV, G, F) {
        var n = PRIV.p.getlen();
        var g = new FF_WWW(2 * n);

        FF_WWW.fromBytes(g, G);
        ...
        g.dscopy(jp);
        jp.mod(PRIV.q);
        if (FF_WWW.comp(jp, jq) > 0) jq.add(PRIV.q);
        jq.sub(jp);
        jq.norm();

        var t = FF_WWW.mul(PRIV.c, jq);
        jq = t.dmod(PRIV.q);

        t = FF_WWW.mul(jq, PRIV.p);
        g.add(t);
        g.norm();
