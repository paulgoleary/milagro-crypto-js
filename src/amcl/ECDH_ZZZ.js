...
//		var T=[];
		...
		gx=new BIG_XXX(0); gx.rcopy(ROM_CURVE_ZZZ.CURVE_Gx);

		if (ECP_ZZZ.CURVETYPE!=ECP_ZZZ.MONTGOMERY)
		{
			gy=new BIG_XXX(0); gy.rcopy(ROM_CURVE_ZZZ.CURVE_Gy);
			G.setxy(gx,gy);
		}
		else G.setx(gx);

		r=new BIG_XXX(0); r.rcopy(ROM_CURVE_ZZZ.CURVE_Order);
	...
	ECPSP_DSA: function(sha,RNG,S,F,C,D)
	{
		...
		var B=this.hashit(sha,F,0,null,BIG_XXX.MODBYTES);
		...
		do {
			...
			u=BIG_XXX.modmul(u,w,r);
			u.invmodp(r);
			d=BIG_XXX.modmul(s,c,r);
			d.add(f);
			d=BIG_XXX.modmul(d,w,r);
			d=BIG_XXX.modmul(u,d,r);
		} while (d.iszilch());
	...
	ECPVP_DSA: function(sha,W,F,C,D)
	{
		...
		B=this.hashit(sha,F,0,null,BIG_XXX.MODBYTES);
		...
		if (res===0)
		{
			...
			else
			{
				...
				else
				{
					d=P.getX();
					d.mod(r);
					if (BIG_XXX.comp(d,c)!==0) res=this.INVALID;
				}
			}
		}
