...
/* set this=(x,y) where x and y are BIGs */
	setxy: function(ix,iy) 
	{
		...
		this.x=new ctx.FP(0); this.x.bcopy(ix);
		var bx=this.x.redc();

		this.y=new ctx.FP(0); this.y.bcopy(iy);
		this.z=new ctx.FP(1);
		...
		{
			var y2=new ctx.FP(0); y2.copy(this.y);
			y2.sqr();
...
/* this+=this */
	dbl: function() 
	{
		if (ECP.CURVETYPE==ECP.WEIERSTRASS)
		{
			...
			{
				var t0=new ctx.FP(0); t0.copy(this.y); //FP t0=new FP(y);                      /*** Change ***/    // Edits made
				t0.sqr();
				var t1=new ctx.FP(0); t1.copy(this.y); //FP t1=new FP(y);
				t1.mul(this.z);
				var t2=new ctx.FP(0); t2.copy(this.z); //FP t2=new FP(z);
				t2.sqr();
				...
				var x3=new ctx.FP(0); x3.copy(t2); //FP x3=new FP(t2);
				x3.mul(this.z);
				var y3=new ctx.FP(0); y3.copy(t0); //FP y3=new FP(t0);
				y3.add(t2); y3.norm();
				this.z.mul(t1); 
			...
			{
				var t0=new ctx.FP(0); t0.copy(this.x); //FP t0=new FP(x);
				var t1=new ctx.FP(0); t1.copy(this.y);//FP t1=new FP(y);
				var t2=new ctx.FP(0); t2.copy(this.z);//FP t2=new FP(z);
				var t3=new ctx.FP(0); t3.copy(this.x);//FP t3=new FP(x);
				var z3=new ctx.FP(0); z3.copy(this.z);//FP z3=new FP(z);
...
//System.out.println("Out of dbl");
		...
		if (ECP.CURVETYPE==ECP.EDWARDS)
		{
			var C=new ctx.FP(0); C.copy(this.x); //FP C=new FP(x);
			var D=new ctx.FP(0); D.copy(this.y);//FP D=new FP(y);
			var H=new ctx.FP(0); H.copy(this.z);//FP H=new FP(z);
...
//System.out.println("Out of dbl");
		...
		if (ECP.CURVETYPE==ECP.MONTGOMERY)
		{
			var A=new ctx.FP(0); A.copy(this.x);  //FP A=new FP(x);
			var B=new ctx.FP(0); B.copy(this.x);//FP B=new FP(x);		
			...
			A.copy(C); A.imul((ctx.ROM_CURVE.CURVE_A+2)>>2);
...
//	System.out.println("Into add");                      // Edits made
				...
				var t4=new ctx.FP(0); t4.copy(Q.x);//FP t4=new FP(Q.x);
				t4.add(Q.y); t4.norm();
				t3.mul(t4);
				...
				var x3=new ctx.FP(0); x3.copy(Q.y);//FP x3=new FP(Q.y);
				x3.add(Q.z); x3.norm();
				...
				var y3=new ctx.FP(0); y3.copy(Q.x);//FP y3=new FP(Q.x);
				y3.add(Q.z); y3.norm();
				x3.mul(y3);
				y3.copy(t0);
				y3.add(t2);
				...
				var z3=new ctx.FP(0); z3.copy(t1);//FP z3=new FP(t1); 
				z3.add(t2); z3.norm();
				t1.sub(t2); t1.norm(); 
				y3.imul(b);
...
//System.out.println("Out of add");
		...
		if (ECP.CURVETYPE==ECP.EDWARDS)
		{
			...
			var A=new ctx.FP(0); A.copy(this.z);  //FP A=new FP(z);
			var B=new ctx.FP(0); //FP B=new FP(0);
			var C=new ctx.FP(0); C.copy(this.x);//FP C=new FP(x);
			var D=new ctx.FP(0); D.copy(this.y);//FP D=new FP(y);
			...
			if (ctx.ROM_CURVE.CURVE_B_I==0)
			{
				var b=new ctx.FP(0); b.rcopy(ctx.ROM_CURVE.CURVE_B);
				E.mul(b);
			}
			else
...
/* Differential Add for Montgomery curves. this+=Q where W is this-Q and is affine. */
	dadd: function(Q,W) 
	{
		var A=new ctx.FP(0); A.copy(this.x);
		var B=new ctx.FP(0); B.copy(this.x);
		var C=new ctx.FP(0); C.copy(Q.x);
		var D=new ctx.FP(0); D.copy(Q.x);
		var DA=new ctx.FP(0);
		var CB=new ctx.FP(0);	
...
ECP.fromBytes= function(b)
{
	var i,t=[];
	var P=new ECP();
	var p=new ctx.BIG(0); p.rcopy(ctx.ROM_FIELD.Modulus);
