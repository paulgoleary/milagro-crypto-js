...
/* Hard part of final exp */
		if (ctx.ECP.CURVE_PAIRING_TYPE==ctx.ECP.BN)
		{
			...
			x5=new ctx.FP12(x2); /*x5.copy(x2);*/  x5.conj();
			lv=x2.pow(x);
...
// Ghamman & Fouotsa Method
			y0=new ctx.FP12(r); y0.usqr();
			y1=y0.pow(x);
			x.fshr(1); y2=y1.pow(x); x.fshl(1);
			y3=new ctx.FP12(r); y3.conj();
			y1.mul(y3);
...
PAIR.glv= function(e)
{
	...
	if (ctx.ECP.CURVE_PAIRING_TYPE==ctx.ECP.BN)
	{
		var i,j;
		var t=new ctx.BIG(0);
		var q=new ctx.BIG(0); q.rcopy(ctx.ROM_CURVE.CURVE_Order);
		var v=[];
...
PAIR.G1mul= function(P,e)
{
	...
	if (ctx.ROM_CURVE.USE_GLV)
	{
		...
		Q.copy(P);
		var q=new ctx.BIG(0); q.rcopy(ctx.ROM_CURVE.CURVE_Order);
		var bcru=new ctx.BIG(0); bcru.rcopy(ctx.ROM_CURVE.CURVE_Cru);
...
PAIR.G2mul= function(P,e)
{
	...
	if (ctx.ROM_CURVE.USE_GS_G2)
	{
		...
		var i,np,nn;
		P.affine();
		Q[0]=new ctx.ECP2(); Q[0].copy(P);
		for (i=1;i<4;i++)
		{
			Q[i]=new ctx.ECP2(); Q[i].copy(Q[i-1]);
			Q[i].frob(f);
		}
...
PAIR.GTpow= function(d,e)
{
	...
	if (ctx.ROM_CURVE.USE_GS_GT)
	{
		...
		for (i=1;i<4;i++)
		{
			g[i]=new ctx.FP12(0); g[i].copy(g[i-1]);
			g[i].frob(f);
		}
		for (i=0;i<4;i++)
		{
