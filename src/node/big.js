...
//if (!b.isok()) alert("Problem in mul b");
		...
		{
			s-=d[k-BIG.NLEN]; t=s; for (i=BIG.NLEN-1;i>=1+Math.floor(k/2);i--) t+=(a.w[i]-a.w[k-i])*(b.w[k-i]-b.w[i]); c.w[k]=t; 
		}
...
//if (!a.isok()) alert("Problem in sqr");
	...
	{
		t=a.w[BIG.NLEN-1]*a.w[j-BIG.NLEN+1]; for (var i=j-BIG.NLEN+2; i<(j+1)>>1; i++) {t+=a.w[j-i]*a.w[i];} t+=t;  
		c.w[j]=t; j++;
		t=a.w[BIG.NLEN-1]*a.w[j-BIG.NLEN+1]; for (var i=j-BIG.NLEN+2; i<(j+1)>>1; i++) {t+=a.w[j-i]*a.w[i];} t+=t; t+=a.w[j>>1]*a.w[j>>1]; 
		c.w[j]=t; j++;
	}
...
BIG.monty=function(m,nd,d)
{
	...
	t=d.w[0]; v[0]=((t&BIG.BMASK)*nd)&BIG.BMASK; t+=v[0]*m.w[0]; 
	c=d.w[1]+(t*BIG.MODINV); s=0;

	for (var k=1;k<BIG.NLEN;k++)
	{
		t=c+s+v[0]*m.w[k];
		for (i=k-1;i>Math.floor(k/2);i--) t+=(v[k-i]-v[i])*(m.w[i]-m.w[k-i]);
		v[k]=((t&BIG.BMASK)*nd)&BIG.BMASK; t+=v[k]*m.w[0]; 
