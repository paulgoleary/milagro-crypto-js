...
/* Test P == Q */
	...
	equals: function(Q) {
		...
		var a=new ctx.FP2(0); a.copy(this.x);
		var b=new ctx.FP2(0); b.copy(Q.x);
...
/* this+=this */
	dbl: function() 
	{
		...
		var iy=new ctx.FP2(0); iy.copy(this.y); //FP2 iy=new FP2(y);
		iy.mul_ip(); iy.norm();

		var t0=new ctx.FP2(0); t0.copy(this.y);//FP2 t0=new FP2(y);                  //***** Change 
		t0.sqr();            t0.mul_ip();   
		var t1=new ctx.FP2(0); t1.copy(iy);//FP2 t1=new FP2(iy);  
		t1.mul(this.z);
		var t2=new ctx.FP2(0); t2.copy(this.z);//FP2 t2=new FP2(z);
		t2.sqr();
		...
		var x3=new ctx.FP2(0); x3.copy(t2);//FP2 x3=new FP2(t2);
		x3.mul(this.z); 

		var y3=new ctx.FP2(0); y3.copy(t0); //FP2 y3=new FP2(t0);   
...
/* this+=Q */
	add: function(Q) 
	{
		...
		var t0=new ctx.FP2(0); t0.copy(this.x); //FP2 t0=new FP2(x);
		t0.mul(Q.x);         // x.Q.x
		var t1=new ctx.FP2(0); t1.copy(this.y); //FP2 t1=new FP2(y);
		t1.mul(Q.y);		 // y.Q.y

		var t2=new ctx.FP2(0); t2.copy(this.z);//FP2 t2=new FP2(z);
		t2.mul(Q.z);
		var t3=new ctx.FP2(0); t3.copy(this.x);//FP2 t3=new FP2(x);
		t3.add(this.y); t3.norm();          //t3=X1+Y1
		var t4=new ctx.FP2(0); t4.copy(Q.x);//FP2 t4=new FP2(Q.x);            
		...
		var x3=new ctx.FP2(0); x3.copy(Q.y);//FP2 x3=new FP2(Q.y);
		...
		var y3=new ctx.FP2(0); y3.copy(Q.x);//FP2 y3=new FP2(Q.x);				
		...
		var z3=new ctx.FP2(0); z3.copy(t1);//FP2 z3=new FP2(t1); 
		z3.add(t2); z3.norm();
		t1.sub(t2); t1.norm(); 
		y3.imul(b); 
...
ECP2.RHS=function(x) 
{
	...
	var c=new ctx.BIG(0); c.rcopy(ctx.ROM_CURVE.CURVE_B);
