...
/* configure PIN here */
	...
	TS:12,        /* 10 for 4 digit PIN, 14 for 6-digit PIN - 2^TS/TS approx = sqrt(MAXPIN) */
	TRAP:2000,     /* 200 for 4 digit PIN, 2000 for 6-digit PIN  - approx 2*sqrt(MAXPIN) */
...
/* Note that u and v are indistinguisible from random strings */
	ENCODING: function(rng,E)
	{
		...
		var P=new ctx.ECP(0); P.setxy(u,v);
...
/* create random secret S */
	...
	{
		var r=new ctx.BIG(0); r.rcopy(ctx.ROM_CURVE.CURVE_Order);
...
/* Extract Server Secret SST=S*Q where Q is fixed generator in G2 and S is master secret */
	GET_SERVER_SECRET: function(S,SST)
	{
		...
		var A=new ctx.BIG(0);
		var B=new ctx.BIG(0);
		A.rcopy(ctx.ROM_CURVE.CURVE_Pxa); B.rcopy(ctx.ROM_CURVE.CURVE_Pxb);
		var QX=new ctx.FP2(0); QX.bset(A,B);
		A.rcopy(ctx.ROM_CURVE.CURVE_Pya); B.rcopy(ctx.ROM_CURVE.CURVE_Pyb);
